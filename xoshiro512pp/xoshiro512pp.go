/*
Package xoshiro512pp implements xoshiro512++ 1.0, one of our all-purpose, rock-solid
...
//
// Go port of http://prng.di.unimi.it/xoshiro512plusplus.c
type Source struct {
	state [8]uint64
}
...
func (s *Source) Uint64() uint64 {
	result := bits.RotateLeft64(s.state[0]+s.state[2], 17) + s.state[2]
	t := s.state[1] << 11
	s.state[2] ^= s.state[0]
	s.state[5] ^= s.state[1]
	s.state[1] ^= s.state[2]
